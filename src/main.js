...
const heroInner = document.getElementById('heroInner'); // <— aggiungi
const nextSection = document.querySelector('.jewel-hero-panel');
const canvas = document.querySelector('.webgl');

function getViewSize() {
  const w = heroInner?.clientWidth || window.innerWidth;
  const h = heroInner?.clientHeight || window.innerHeight;
  return { w, h };
}
...
renderer.setSize(initW, initH, false); // <-- importante: false
...
const smoothMouseWorld = new THREE.Vector3(9999, 9999, 9999);
let hasMouseWorld = false;

// posizione automatica della calamita (usata anche per desktop ora)
...
float halfLambert(vec3 n, vec3 l){ return max(dot(n,l)*0.5+0.5, 0.0); }
...
void main(){
  ...
  vec3 swirl = (uWind + vec3(n-0.5)) * (uNoiseAmp * t);
  ...
  vec3 idle = vec3(i1-0.5, i2-0.5, i3-0.5) * (uIdleAmp * (0.4 + 0.6 * (0.3 + 0.7 * t)));
...
function markSceneReady() {
  if (window.__lumiereReady) return;
  window.__lumiereReady = true;
  window.dispatchEvent(new Event('lumiere:ready'));
}

canvas.style.pointerEvents = 'auto';

// versione mouse -> world (lasciata commentata per ora)
/*
...
gltfLoader.load(
  ...
  (res) => {
    let srcMesh = null;
    res.scene.traverse(o => { if (o.isMesh && !srcMesh) srcMesh = o; });
    if (!srcMesh) {
      addFromMesh(new THREE.Mesh(new THREE.TorusKnotGeometry(1, 0.35, 220, 32)));
      markSceneReady();
    } else {
      srcMesh.updateMatrixWorld(true);
      const meshClone = new THREE.Mesh(srcMesh.geometry.clone(), new THREE.MeshBasicMaterial());
...
let heroLocked = true;         // finché true, la pagina NON deve scendere
const HERO_SENS = 0.0009;      // sensibilità wheel (tarala)

let wasLocked = true;

function setHeroProgress(p) {
  heroProgress = THREE.MathUtils.clamp(p, 0, 1);

  const SHOW_OBJECT00_AT = 0.78;

  if (window.__setTopLabel) {
    if (heroProgress >= SHOW_OBJECT00_AT) window.__setTopLabel('OBJECT 00');
    else window.__setTopLabel('SCROLL DEEP AND SLOWLY');
  }


  // guidi l’animazione ESISTENTE
  P.uProgress.value = heroProgress;
  applyLogoProgress(heroProgress);

  // UI panel: appare prima, mentre finisci il portale
  if (nextSection) {
    const START_UI = 0.50;  // prima era 0.88
    const END_UI   = 0.95;  // finisce prima di 1
    const t = THREE.MathUtils.clamp((heroProgress - START_UI) / (END_UI - START_UI), 0, 1);

    nextSection.style.opacity = t.toFixed(3);
    const translateY = 18 * (1 - t);
    nextSection.style.transform = `translate(-50%, ${translateY}vh)`;
    nextSection.style.pointerEvents = t > 0.01 ? 'auto' : 'none';
  }

  // lock/unlock
  if (heroProgress <= 0) heroLocked = true;
  if (heroProgress >= 1) heroLocked = false;

  // ✅ quando sblocchi per la prima volta: snap alla fine della hero
  if (wasLocked && !heroLocked) {
    const startY = heroSection?.offsetTop || 0;
    const endY = startY + (heroSection?.offsetHeight || 0) - window.innerHeight;
    window.scrollTo(0, Math.max(0, endY));
  }

  wasLocked = heroLocked;

}

// esponi stato per lo script di scroll
window.__heroLock = {
  get locked() { return heroLocked; },
  get progress() { return heroProgress; },
  setProgress: setHeroProgress
};
...
// utile perché su schermi grandi cambia anche solo il container, non sempre "resize window"
if (heroInner && 'ResizeObserver' in window) {
  new ResizeObserver(resizeRenderer).observe(heroInner);
}
...
function tick() {
  ...
  // --- calamita / "bolla" ---
  // Ora sia su mobile che su desktop si muove in autonomia
  const t = P.uTime.value * 0.35;
  pivot.getWorldPosition(tmpWorldPos);
  const radiusX = 0.7;
  const radiusY = 1;
  ...
  // --- vecchia versione desktop con il mouse (lasciata commentata per debug futuro) ---
  /*
  if (!IS_MOBILE) {
    if (hasMouseWorld) {
